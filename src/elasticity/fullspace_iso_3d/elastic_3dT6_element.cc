...
#include "tensor_utilities_3DT6.h"

#include "elastic_3dT6_element.h"
#include "h_potential_3DT6.h"
...
il::StaticArray2D<double, 6, 18> make_local_3dbem_submatrix(
    const int kernel_id, double shear_m, double poiss_r, double h,
    std::complex<double> z, const il::StaticArray<std::complex<double>, 3> &tau,
  ...
  bool IsDegen = il::abs(d[0]) < bie::h_tol ||
                 abs(d[1]) < bie::h_tol || // moved form abs to il::abs() CP2021
                 il::abs(d[2]) < bie::h_tol; // (d[0]*d[1]*d[2]==0); // moved
                                             // form abs to il::abs() CP2021
  ...
  for (int m = 0; m < 3; ++m) {
    ...
    if (il::abs(dm) >= bie::h_tol && !is_90_ang(0, m) &&
        !is_90_ang(1, m)) { // moved form abs to il::abs() CP2021
      std::complex<double>
          // exp(I * chi(0, m))
      ...
      if (fabs(h) < bie::h_tol) {
        il::StaticArray3D<std::complex<double>, 6, 4, 3>
            s_incr_n = bie::s_integral_lim(kernel_id, poiss_r, eixn, dm),
  ...
  il::StaticArray3D<std::complex<double>, 6, 4, 3> s_ij_infl_nod =
      dot(sfm_z, s_ij_infl_mon);
  // s_ij_infl_nod(nodes of source elem,stress component in complex not., dd
  // compoment)
  ...
  for (int j = 0; j < 6; ++j) // looping over the nodes of the source elem
  {
    ...
    for (int k = 0; k < 3; ++k) // looping over the components of dd
    {
      ...
      stress_el_2_el_infl(0, q + k) =
          scale * (real(s_ij_infl_nod(j, 0, k)) + real(s_ij_infl_nod(j, 1, k)));
...
il::StaticArray4D<std::complex<double>, 6, 4, 3, 9>
s_integral_gen(const int kernel_id, double poiss_r, std::complex<double> eix,
...
il::StaticArray4D<std::complex<double>, 6, 4, 3, 5>
s_integral_red(const int kernel_id, double poiss_r, std::complex<double> eix,
               double h) {
...
il::StaticArray3D<std::complex<double>, 6, 4, 3>
s_integral_lim(const int kernel_id, double poiss_r, std::complex<double> eix,
...
il::StaticArray<std::complex<double>, 9>
integral_cst_fun(double h, std::complex<double> d, double a, double x,
                 std::complex<double> eix) {

  double abs_d = std::abs(d), d2 = abs_d * abs_d, a2 = a * a,
         r = std::sqrt(h * h + a2 + d2), r2 = r * r, r3 = r2 * r, r5 = r3 * r2,
         ar = a / r, ar2 = ar * ar, hr = std::fabs(h / r), b = 1.0 / (r2 - a2),
         b2 = b * b, b3 = b2 * b;
  double tah_x = std::imag(eix) / std::real(eix), tr = hr * tah_x,
         g0 = std::atan(tr), f0 = std::atanh(ar), f1 = -0.5 * ar * b,
         f2 = 0.25 * (3.0 - ar2) * ar * b2,
...
il::StaticArray<std::complex<double>, 5>
integral_cst_fun_red(double h, std::complex<double> d, double a) {

  double h2 = h * h, h4 = h2 * h2, h6 = h4 * h2, abs_d = std::abs(d),
         d2 = abs_d * abs_d, a2 = a * a, ro = std::sqrt(a2 + d2),
         r = std::sqrt(h2 + a2 + d2), rr = ro / r, rr2 = rr * rr,
         rr4 = rr2 * rr2, f0 = std::atanh(rr), f1 = -0.5 * rr / h2,
         f2 = 0.25 * (3.0 - rr2) * rr / h4,
...
il::Array2D<double> traction_influence_3DT6(
    ...
    bie::ElasticProperties const &elas_, il::int_t I_want_global_DD,
  ...
  // now we have to transpose due to different conventions of Mesh3D classes and
  // functions to compute the quadratic triangular element kernel
  for (int j = 0; j < 3; ++j) {
    for (int i = 0; i < 3; ++i) {
  ...
  // from cartesian global coord to local complex repr. coord. (only 1st 2
  // component the 3rd component is 0 because of the translation)
  ...
  // now we have to transpose due to different conventions of
  // Mesh3D/TriangularElementData classes and functions to compute the quadratic
  // triangular element kernel
  for (int j = 0; j < 3; ++j) {
    for (int i = 0; i < 3; ++i) {
  ...
  // Rotation tensor for the target element (the same function as for the source
  // is being used)
  ...
  il::StaticArray<il::StaticArray<double, 3>, 6> el_cp_crd =
      el_cp_uniform(el_vert_t, elem_data_r.getBeta1(), elem_data_r.getBeta2());
  ...
  // w.r. to the source element's local coordinate system (both DD and
  // stresses!)
  il::StaticArray2D<double, 6, 18> stress_infl_el2p_loc_h =
      make_local_3dbem_submatrix(1, elas_.shear_modulus(),
                                 elas_.poisson_ratio(), hz.h, hz.z, tau, sfm);
  ...
  // we obtain traction in the collocation point in the local system of the
  // source point
  //
  // trac_el2p_loc--> traction and DD are both in local system of the SOURCE
  // element
  //

  il::StaticArray2D<double, 3, 18> trac_cp_local2global =
      il::dot(r_tensor_s, il::Dot::Transpose, trac_el2p_loc);
  // 3 component of traction vs 6 source nodes * 3 DD components
  // so 3*18 <(1 coll. point vs 6 source nodes)>
  //
  // trac_cp_local2global--> traction and DD are in the local system of the
  // source element ,traction are in global coord. system (for all the nodes of
  // the source)
  //
  //
  // taking a block (one node of the "source" element)
  // traction matrix
  // t_dir_x_node(n_t)_dd1_on_node_(n_s)  t_dir_x_node(n_t)_dd2_on_node_(n_s)
  // t_dir_x_node(n_t)_dd3_on_node_(n_s) t_dir_y_node(n_t)_dd1_on_node_(n_s)
  // t_dir_y_node(n_t)_dd2_on_node_(n_s)  t_dir_y_node(n_t)_dd3_on_node_(n_s)
  // t_dir_z_node(n_t)_dd1_on_node_(n_s)  t_dir_z_node(n_t)_dd2_on_node_(n_s)
  // t_dir_z_node(n_t)_dd3_on_node_(n_s)
  //
  ...
  if (I_want_global_DD == 0 && I_want_global_traction == 1) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        NodeDDtriplet_to_CPtraction_influence_matrix(k, j) =
            trac_infl_n2p_local2global(k, j);
      }
    }
  }
  //
  // trac_infl_n2p_local2global--> traction and DD are in the local system of
  // the source element ,traction are in global coord. system
  //
  //
  ...
  if (I_want_global_DD == 0 && I_want_global_traction == 0) {
    ...
    //
    // trac_infl_n2p_local2local--> traction and DD are in the local system of
    // the source element ,traction are in local system of the target
    //
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        NodeDDtriplet_to_CPtraction_influence_matrix(k, j) =
            trac_infl_n2p_local2local(k, j);
      }
    }
  }
  ...
  if (I_want_global_DD == 1 && I_want_global_traction == 0) {
    ...
    trac_infl_n2p_global2local =
        il::dot(trac_infl_n2p_local2global, r_tensor_s);
    trac_infl_n2p_global2local =
        il::dot(r_tensor_t, trac_infl_n2p_global2local);
    //
    // trac_infl_n2p_global2local--> traction and DD are in the global system of
    // the source element ,traction are in local system of the target
    //
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        NodeDDtriplet_to_CPtraction_influence_matrix(k, j) =
            trac_infl_n2p_global2local(k, j);
      }
    }
  }
  ...
  if (I_want_global_DD == 1 && I_want_global_traction == 1) {
    ...
    trac_infl_n2p_global2global =
        il::dot(trac_infl_n2p_local2global, r_tensor_s);
    //
    // trac_infl_n2p_global2global--> traction and DD & traction are both in the
    // global system
    //
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        NodeDDtriplet_to_CPtraction_influence_matrix(k, j) =
            trac_infl_n2p_global2global(k, j);
      }
    }
  }
